...
import { ScidarEmail } from './scidar-email';
...
export class ResearcherService {
  ...
  private researcherServiceURL = 'http://localhost:8081/researchers';
  ...
  getResearcher(id: string): Observable<Researcher> {
    const url = `${this.researcherServiceURL}/getresearcher/${id}`;
    return this.http.get<Researcher>(url).pipe(
      tap(_ => this.log(`fetchovao istrazivaca id=${id}`)),
      catchError(this.handleError<Researcher>(`getResearcher id=${id}, url=${url}`))
    );
  }
