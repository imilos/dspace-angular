...
import { ResearcherService } from 'src/app/unic/researcher.service';
import { ScidarEmail } from 'src/app/unic/scidar-email';
...
export class EmailPrijavaComponent {
  ...
  constructor(private AuthService: AuthService, private researcherService: ResearcherService,
    private EPersonService: EPersonDataService, private NotificationService: NotificationsService 
  ) { }
  ...
  reportErrorInItem(): void {
    ...
    if (this.user_email != null) {
      this.researcherService.reportErrorInItem(scidar_email)
        .subscribe( {
          next: () => { 
            this.NotificationService.success("The email has been sucessfully sent to the administrator with CC to " + this.user_email);
            this.reportErrorInItemForm.controls.note.reset();
          }, 
          error: () => { 
            this.NotificationService.error("Cannot send the email.");
          }
        });
    }
